    ...
    <>
      ...
      {isLoading ? (
        ...
        <ul className="users-list">
          {
            items.filter(obj => {
              const fullName = (obj.first_name + "" + obj.last_name).toLowerCase().replace(/\s+/g, '');
              // replace - to right concatination
              const searchValueNormalized = searchValue.toLowerCase().replace(/\s+/g, '');
              return (
                fullName.includes(searchValueNormalized) || 
                obj.email.toLowerCase().includes(searchValueNormalized));
            })
            .map((obj) => (
